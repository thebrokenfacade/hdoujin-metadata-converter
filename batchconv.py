# This script should convert any info.txt in the same dir or any sub-dir to an info.json in the respective dir
# This script requires python3 to be installed in order to run properly.
# To run the script place batchconv.py in the directory containing the info.txt you want to convert
...
def convinfo(gal_path, file_path):
    ...
    # Open a new json file to write into and create the json line by line
    with open(gal_path + "\\info.json", "w", encoding='utf8') as out_file:
        out_file.write("{\n")
...
for root, dirs, files in os.walk(root):
    for file in files:
        if file == "info.txt":
            ...
            convinfo(gal_path, file_path)
            count = count + 1


if count == 0:
    print("\nThere were no info.txt files found in the current dir or any sub-dirs.")
    print("    There is nothing to do!")
    input("\nPress enter to exit...")
    exit()
else:
    if count == 1:
        print("\nThere was %i info.txt file found in the scanned dirs." % count)
        print("    A total of %i info.json file was created in it's respective dir." % count)
        print("    The info.txt file was not deleted.")
    else:
        print("\nThere were %i info.txt files found in the scanned dirs." % count)
        print("    A total of %i info.json files were created in their respective dirs." % count)
        print("    The info.txt files were not deleted.")
